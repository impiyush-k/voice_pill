...
def ensure_single_instance() -> object | None:
    ...
    Returns the mutex handle (must stay alive) or None if another instance exists.
    """
    try:
        mutex_name = "VoicePill_SingleInstance_Mutex_v1"
...
class VoicePillApp:
    ...
    def _register_hotkeys(self):
        ...
        # Ctrl+Space — activate batch mode (always registered)
        success = self.hotkey_manager.register(
            "activate", HOTKEY_ACTIVATE, self._on_activate_from_thread
        )
        if not success:
            self.tray.show_notification(
                "Voice Pill",
                f"Failed to register hotkey {HOTKEY_ACTIVATE}. "
                "Try running as administrator.",
            )

        # Ctrl+Shift+Space — activate streaming dictation mode (always registered)
        stream_success = self.hotkey_manager.register(
            "stream_activate", HOTKEY_STREAM_ACTIVATE, self._on_stream_activate_from_thread
        )
        if not stream_success:
            self.tray.show_notification(
                "Voice Pill",
                f"Failed to register streaming hotkey {HOTKEY_STREAM_ACTIVATE}.",
            )
    ...
    def _register_stop_hotkey(self):
        ...
        if not self._space_registered:
            self.hotkey_manager.register(
                "stop", HOTKEY_STOP, self._on_stop_from_thread
            )
    ...
    def _on_activate(self):
        ...
        If IDLE or READY → start batch recording.
        """
        ...
        if state in (self.State.IDLE, self.State.READY):
            # Silently refresh devices so we instantly catch newly paired Bluetooth headsets
...
def main():
    ...
    if mutex is None:
        print("Voice Pill is already running.")
        sys.exit(0)
